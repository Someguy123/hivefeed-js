...
// import {Account, Blockchain, KeyPair} from './lib/chain';
...
try {
    var accountmgr = new Account(config.name, config.wif, config.network, config.nodes);
} catch(e) {
    console.error('A serious error occurred while checking your account:', e);
    process.exit(1);
}
...
function main() {
    get_price((err,price) => {
        ...
        if(!dryRun) {
            accountmgr.publish_feed_set_props(price);
        } else {
