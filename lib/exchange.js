...
/**
 * Check if a pair can be gotten via a proxy (e.g. hive/usd via hive/btc -> btc/usd)
 * @param {str} from - From coin
 * @param {str} to - Destination coin
...
/**
 * Attempt to handle a pair using a proxy (e.g. hive/usd via hive/btc -> btc/usd)
 * @param {str} from - From coin
 * @param {str} to - Destination coin
...
function try_proxy(from,to,proxy,callback) {
    ...
    var promises = [
         ...
         * E.g. Hive->USD via BTC - this would handle Hive->BTC
         */
         ...
         * E.g. Hive->USD via BTC - this would handle BTC->USD
         */
        new Promise(function(resolve,reject) { 
 ...
 * 
 * Example: get_pair('hive', 'usd', function(err, price) { log('hive/usd:', price) })
 * -> Check if direct path (hive/usd) possible using get_adapters(from,to,callback)
 * -> If not, try an inverted path (usd/hive) with get_reverse_adapters(from,to,callback)
 * -> If no reverse path possible, call find_proxy(from,to) to see if a proxy is possible
 *    for example hive->btc->usd
 ...
 *    Nested proxies are possible, for example hive -> btc -> usdt -> usd
...
function get_pair(from,to,callback) {
    ...
    .then(
        function() { 
            ...
            dbg_log(green(`[get_pair] (${from}/${to}) - Median average price: ${avg.toFixed(8)} ${to.toUpperCase()} per ${from.toUpperCase()}`))
...
/**
 * Convert a list of ``provides`` pairs, into a readable string
 * e.g. ``hive:btc, hbd:btc, hive:btc, btc:usdt, usdt:usd``
 *
 * @param {Array<Array<string>>} data 
 */
function readable_provides(data) {
    var out = '';
...
var ex_provide_block = settings.config.exchanges_no_provide,
    ex_provide_add = settings.config.exchanges_provide;

_get_blocked_pairs = (code) => (code in ex_provide_block) ? ex_provide_block[code] : [];
_get_added_pairs = (code) => (code in ex_provide_add) ? ex_provide_add[code] : [];

/**
 * Remove the coin pair ``pair`` from the ``provides`` list of the adapter ``adapter``.
 * 
 * @param {*} adapter - An exchange adapter such as BittrexAdapter / BinanceAdapter
 * @param {Array<string>} pair - A coin pair, e.g. ``['hive', 'btc']``
 */
function _rm_pair(adapter, pair) {
    var a = adapter;
    if (adapter.has_pair(pair[0], pair[1])) {
        dbg_log(`Removing pair ${pair[0]}/${pair[1]} from adapter '${adapter.code}'`);
        adapter.provides = adapter.provides.filter(pv => pv[0] != pair[0] && pv[1] != pair[1]);
        dbg_log(`New provides for '${adapter.code}': ${readable_provides(adapter.provides)} `);
    }
}

// Iterate through ``available_adapter``, skipping disabled adapters, and handling
// added/blocked pairs from the user config - then initialise enabled adapters.
for (var a of available_adapters) {

    if(settings.config.disable_exchanges.indexOf(a.code) > -1) {
        log(`Exchange adapter for '${a.name}' is disabled. Not initialising adapter.`);
    } else {
        // Remove any disabled pairs specified in config.exchanges_no_provide
        for (var pair of _get_blocked_pairs(a.code)) {
            _rm_pair(a, pair);
        }
        // Add any additional pairs specified in config.exchanges_provide
        for (var pair of _get_added_pairs(a.code)) {
            dbg_log(`Adding extra pair ${pair[0]}/${pair[1]} to adapter '${a.code}'`);
            a.provides.push(pair);
        }

        var rp = readable_provides(a.provides);
        dbg_log(`Initialising adapter for exchange '${a.name}' - provides pairs: ${rp}`);
        add_adapter(a);
    }
}
...
//
// exchange.get_pair('hive','usd', function(err, price) {
